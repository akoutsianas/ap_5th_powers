...
function associate_newforms(fnew,Newfsb)


	for i in [1..#Newfsb] do
		find := true;
		fnewb := Newfsb[i][1];		

		for q in PrimesInInterval(11,200) do
			ZZ := RingOfIntegers(BaseField(fnew));
			aqfnew := HeckeEigenvalue(Eigenform(fnew),q*ZZ);
			aqfnewb := Coefficient(fnewb,q);
			if MinimalPolynomial(aqfnew) ne MinimalPolynomial(aqfnewb) then
				find := false;
				break;
			end if;
		end for;
